...
public class DatecsUtil {
  ...
  public static int getResource(Context context, String resourceName, String resourceType) {
    // Try app package first (merged resources)
    int resId = context.getResources().getIdentifier(resourceName, resourceType, context.getPackageName());

    // Fallback to plugin package namespace used by Capacitor Cordova bridge
    if (resId == 0) {
      resId = context.getResources().getIdentifier(resourceName, resourceType, "capacitor.cordova.android.plugins");
    }

    // Fallback to original plugin package name (Cordova)
    if (resId == 0) {
      resId = context.getResources().getIdentifier(resourceName, resourceType, "com.giorgiofellipe.datecsprinter");
    }
  ...
  public static String getStringFromStringResource(Context context, String resourceName) {
    int id = DatecsUtil.getStringResource(context, resourceName);
    // Avoid crash if resource is still missing
    if (id == 0) {
      return resourceName;
    }
